...
public class SearchMails extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
                mails = dao.getReceivedMailsForUserFrom(currentUserId, dao.getUserId(username));
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }

        session.setAttribute("search", mails);
        request.getRequestDispatcher("jspFiles/displaySearchMails.jsp").forward(request, response);
